...
import { Search, Filter, X, Sparkles, Grid, Eye } from 'lucide-react';
    ...
    <section className="px-6 max-w-7xl mx-auto py-20" id="productos">
      ...
      >
        <span className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-primary/10 text-primary text-xs font-sans uppercase tracking-widest rounded-full mb-3">
          ...
          {t('catalogTitle')} • {t('piecesCount', { count: artworks.length })}
        </span>
      ...
      {artworks.length === 0 ? (
        ...
        >
          ...
          <h3 className="text-lg font-serif text-textBase mb-2">{t('emptyCatalogTitle')}</h3>
        ...
        <>
          ...
          >
            ...
            <div className="mt-6 text-center">
              <p className="text-textBase/60 text-xs sm:text-sm">
                {t('showing')} <span className="font-semibold text-primary">{displayedProducts.length}</span> {t('of')}{' '}
                <span className="font-semibold text-textBase">{filteredProducts.length}</span> {t('pieces')}
                {searchQuery && (
          ...
          {filteredProducts.length === 0 && (
            ...
            >
              ...
              <h3 className="text-lg font-serif text-textBase mb-2">{t('noResultsTitle')}</h3>
